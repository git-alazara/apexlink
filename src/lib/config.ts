...
export const RULES_VERSION = "2026-08-30";

export const INITIAL_PRICE_CENTS = Number(process.env.INITIAL_PRICE_CENTS ?? 100);
export const PRICE_INCREMENT_CENTS = Number(process.env.PRICE_INCREMENT_CENTS ?? 1);
export const PRICE_DECAY_DAYS = Number(process.env.PRICE_DECAY_DAYS ?? 90);

export function getPriceIncrementCents() {
  return Math.max(1, PRICE_INCREMENT_CENTS);
}
